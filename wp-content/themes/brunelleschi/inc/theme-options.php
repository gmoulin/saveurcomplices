...
function brunelleschi_options_fields($tab) {
	...
	if($tab === 'general'){
		$brunelleschi_options_fields = array(
			...
			'remove-header-image-border' => array(
				'type' => 'checkbox',
				'name' => 'remove-header-image-border',
				'label' => __('Remove Header Image Border?','brunelleschi'),
				'description' => __('Check to Remove Header Image Border.','brunelleschi'),
				'std' => ''
			),
...
function brunelleschi_options_sanitize($input) {
	if( isset($_POST['reset']) && $_POST['reset'] === 'Reset Defaults' ){
		...
		$valid_input['fonts'] = (__('Classic','brunelleschi') || __('Modern','brunelleschi') || __('Nouveau','brunelleschi') === $input['fonts'] ? $input['fonts'] : $valid_input['fonts']);
		...
		$valid_input['navigation-position'] = (__('Nav Above Banner','brunelleschi') || __('Nav Below Banner','brunelleschi') === $input['navigation-position'] ? $input['navigation-position'] : $valid_input['navigation-position']);
